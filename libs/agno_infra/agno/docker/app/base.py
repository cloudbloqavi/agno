...
class DockerApp(InfraApp):
        ...
        container_env.update(
            {
                ...
                "MOUNT_INFRA_DIR": str(self.mount_infra_dir),
    ...
    def get_container_volumes(self, container_context: ContainerContext) -> Dict[str, dict]:
        ...
        if self.infra_root is None:
            logger.error("Invalid infra_root")
            return {}
        ...
        if self.mount_infra_dir:
            infra_root_in_container = container_context.infra_root
            infra_root_on_host = str(self.infra_root)
            logger.debug(f"Mounting: {infra_root_on_host}")
            logger.debug(f"      to: {infra_root_in_container}")
            container_volumes[infra_root_on_host] = {
                "bind": infra_root_in_container,
                "mode": "rw",
            }
        ...
        if self.create_volume:
            ...
            if self.volume_dir is not None:
                volume_host = str(self.infra_root.joinpath(self.volume_dir))
        ...
        if self.mount_resources:
            resources_dir_path = str(self.infra_root.joinpath(self.resources_dir))
        ...
        if self.mount_agno_config:
            ...
            agno_config_container_path = f"{self.infra_dir_container_path}/.config/ag"
