...
def deep_diagnose(pptx_path: str):
    ...
    for slide_idx, slide in enumerate(prs.slides):
        ...
        for shape in slide.shapes:
            ...
            print(
                f"    Position: {shape.left}, {shape.top}  Size: {shape.width}x{shape.height}"
            )
            print(f"    Detected bg: #{bg_hex} (lum={bg_lum:.3f})")

            if getattr(shape, "has_chart", False):
                print(
                    f'    HAS CHART — w={shape.width / Inches(1):.1f}" h={shape.height / Inches(1):.1f}"'
                )
                try:
                    ...
                    print(
                        f"    Chart text elements: {len(defRPrs)} defRPr, {len(rPrs)} rPr"
                    )
            ...
            if getattr(shape, "has_text_frame", False):
                for para_idx, para in enumerate(shape.text_frame.paragraphs):
                    for run_idx, run in enumerate(para.runs):
                        ...
                        print(
                            f"    {status} Run[{para_idx}.{run_idx}]: color={color_src} ratio={ratio:.1f} text='{run.text[:50]}'"
                        )
