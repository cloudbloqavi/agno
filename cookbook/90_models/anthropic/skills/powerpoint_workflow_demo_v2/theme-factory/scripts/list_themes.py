...
def list_themes():
    ...
    if not references_dir.exists():
        return json.dumps({"error": f"References directory not found at {references_dir}"})
    ...
    for file in references_dir.glob("*.md"):
        try:
            ...
            name = lines[0].replace("# ", "").strip() if lines and lines[0].startswith("# ") else file.stem
